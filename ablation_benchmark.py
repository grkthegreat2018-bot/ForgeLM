...
RESULTS_DIR = "research/results/ablation"
...
def get_testable_keys():
    ...
    def apply_rope_share(model):
        from research.keys.rope_share_key import RoPEShareKey
        k = RoPEShareKey()
        k.apply(model)
        return k
    ...
    def apply_per_query_temp(model):
        from research.keys.per_query_temp_key import apply_per_query_temp as _apply
        _apply(model)
        return None
    ...
    def apply_norm_gated_mod(model):
        from research.keys.norm_gated_mod_key import apply_norm_gated_mod as _apply
        _apply(model)
        return None
