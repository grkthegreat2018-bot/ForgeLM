...
Scans the ForgeAI codebase for:
  ...
  3. Architecture keys in forge/keys/ (89 Key classes)
...
class TopicScanner:
    ...
    def _get_scan_files(self) -> list[Path]:
        ...
        # _scan_feature_flags
        files.append(self.root / "forge" / "engine" / "activation.py")
        # _scan_config_params
        files.append(self.root / "forge" / "config.py")
        # _scan_arch_keys
        keys_dir = self.root / "forge" / "keys"
        if keys_dir.exists():
            files.extend(keys_dir.rglob("*_key.py"))
        # _scan_kv_strategies
        files.append(self.root / "forge" / "engine" / "kv_backend.py")
        # _scan_decoding_strategies
        files.append(self.root / "forge" / "engine" / "decoding.py")
        # _scan_quant_modes
        quant_dir = self.root / "forge" / "engine" / "quant"
        if quant_dir.exists():
            files.extend(
                f for f in quant_dir.glob("*.py") if not f.name.startswith("_")
            )
        # _scan_schedulers
        sched_dir = self.root / "forge" / "engine" / "scheduler"
        if sched_dir.exists():
            files.extend(
                f for f in sched_dir.glob("*.py") if not f.name.startswith("_")
            )
        # _scan_training_optimizers
        opt_dir = self.root / "forge" / "training" / "optim"
        if opt_dir.exists():
            files.extend(
    ...
    def _scan_feature_flags(self):
        ...
        path = self.root / "forge" / "engine" / "activation.py"
        if not path.exists():
            return
    ...
    def _scan_config_params(self):
        ...
        path = self.root / "forge" / "config.py"
        if not path.exists():
            return
        content = path.read_text(encoding="utf-8")
    ...
    def _scan_arch_keys(self):
        """Scan forge/keys/ for Key classes."""
        keys_dir = self.root / "forge" / "keys"
    ...
    def _scan_kv_strategies(self):
        ...
        path = self.root / "forge" / "engine" / "kv_backend.py"
        if not path.exists():
            return
        content = path.read_text(encoding="utf-8")
    ...
    def _scan_decoding_strategies(self):
        ...
        path = self.root / "forge" / "engine" / "decoding.py"
        if not path.exists():
            return
        content = path.read_text(encoding="utf-8")
    ...
    def _mark_covered_topics(self):
        ...
        try:
            from forge.evolution.domains import list_domains
