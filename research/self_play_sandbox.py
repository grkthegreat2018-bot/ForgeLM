...
class SelfPlaySandbox:
    ...
    def generate_code(self, prompt: str) -> Tuple[str, Dict]:
        ...
        with torch.inference_mode():
            ...
            for step in range(self.max_gen_tokens):
                ...
                # Stop on [stop] — check if recent generated text contains it
                generated_so_far = self.tokenizer.decode(
                    cur_ids[0, input_ids.shape[1]:], skip_special_tokens=True)
                if stop_str in generated_so_far:
                    break
    ...
    def generate_reasoning(self, prompt: str) -> str:
        ...
        reasoning = self.tokenizer.decode(cur_ids[0, ids.shape[1]:],
