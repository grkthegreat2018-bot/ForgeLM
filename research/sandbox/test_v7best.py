"""Test V7_best checkpoint for coherent output."""
import sys, os
...
# V7_best was saved with V7 config (rank=768), not V7_8B_B (rank=1024)
cfg = get_config("forgelm_v7", device="cuda")
cfg.mtp_n_heads = 2  # may need adjustment
cfg.use_mtp = False
cfg.use_chunked_ce = True
cfg.ce_chunk_size = 128

model = ModelLoader.build_model_fast(
    cfg, checkpoint_path="research/checkpoints/ForgeLM_V7_best.safetensors",
...
# Also test with V7_8B_B config (rank=1024) to see if rank mismatch matters
print("\n\n=== Also testing V7_best with V7_8B_B config (rank=1024) ===")
del model
torch.cuda.empty_cache()

cfg2 = get_config("forgelm_v7_8b_b", device="cuda")
cfg2.mtp_n_heads = 2
cfg2.use_mtp = False
cfg2.use_chunked_ce = True
cfg2.ce_chunk_size = 128

model2 = ModelLoader.build_model_fast(
    cfg2, checkpoint_path="research/checkpoints/ForgeLM_V7_best.safetensors",
    dtype=torch.bfloat16,
)
model2.eval()
...
input_ids = torch.tensor([ids], device="cuda", dtype=torch.long)
with torch.no_grad():
    out = model2(input_ids, targets=None)
    logits = out[0] if isinstance(out, tuple) else out
probs = torch.softmax(logits[0, -1], dim=-1)
top5 = torch.topk(probs, 5)
