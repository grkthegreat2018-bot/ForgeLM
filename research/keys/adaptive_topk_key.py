"""Adaptive Expert Top-K (AdaTopK) â€” entropy-based dynamic expert routing.
...
router logit entropy:
  - Low entropy (confident) â†’ top-1 (50% FFN FLOPs saved for that token)
  - Medium entropy â†’ top-2 (standard)
  - High entropy (uncertain) â†’ top-3 (50% more FLOPs, better quality)

For ForgeLM V2 (4 routed + 1 shared experts, top-2 default):
  - ~40% of tokens are "easy" (low entropy) â†’ top-1
  - ~50% are "medium" â†’ top-2
  - ~10% are "hard" (high entropy) â†’ top-3
...
(merging) â€” AdaTopK changes the ROUTING, not the weights. It's orthogonal
to all other MoE optimizations.

Key class: TRIVIAL â€” runtime routing, training-free, no weight changes.
...
class AdaptiveMoELayer(nn.Module):
    ...
    Instead of fixed top-k, each token gets k âˆˆ [min_k, max_k] based on
    ...
    def __init__(self, moe_layer: nn.Module, min_k: int = 1, max_k: int = 3,
        ...
        Args:
            ...
            lo_entropy: entropy below this â†’ use min_k experts
            hi_entropy: entropy above this â†’ use max_k experts
        """
        super().__init__()
        self.moe = moe_layer
        self.min_k = min_k
    ...
    def _adaptive_k(self, router_logits: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            router_logits: (N, n_experts) â€” raw router logits

        Returns:
            k_per_token: (N,) â€” number of experts to route each token to
        """
        ...
        with torch.no_grad():
            ...
            # Map entropy to k:
            #   entropy < lo_entropy â†’ min_k
            #   entropy > hi_entropy â†’ max_k
            #   in between â†’ linear interpolation
...
class AdaTopKKey(Key):
    """Adaptive Expert Top-K â€” entropy-based dynamic expert routing.
    ...
    Key class: TRIVIAL â€” runtime routing, training-free, reversible.
    """
    ...
    def __init__(self, min_k: int = 1, max_k: int = 3,
        ...
        Args:
            ...
            lo_entropy: normalized entropy below this â†’ min_k
            hi_entropy: normalized entropy above this â†’ max_k
        """
        self.min_k = min_k
        self.max_k = max_k
        self.lo_entropy = lo_entropy
    ...
    def description(self) -> str:
        return (f"Adaptive Expert Top-K (kâˆˆ[{self.min_k},{self.max_k}], "
    ...
    def apply(self, model: nn.Module) -> int:
        ...
        print(f"  [AdaTopK] Patched {n_patched} MoE layers with adaptive top-k "
              f"(kâˆˆ[{self.min_k},{self.max_k}], entropy=[{self.lo_entropy},{self.hi_entropy}])")
        return n_patched
