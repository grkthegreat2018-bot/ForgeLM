"""Cross-Layer KV Sharing (CLKV) — share KV cache across consecutive layer pairs.
...
For ForgeLM V2 (28 layers, 12 heads, head_dim=128, MLA):
  - Standard: 28 layers × 2 (K,V) × 12 heads × T × 128 × 2 bytes (bf16)
  - CLKV:     14 pairs × 2 (K,V) × 12 heads × T × 128 × 2 bytes
  - Savings:  50% KV cache VRAM at all sequence lengths
  - At T=32768: 5.6 GB → 2.8 GB saved
...
Quality: PARTIAL — adjacent layers' KV are correlated but not identical.
...
Key class: TRIVIAL — runtime cache sharing, training-free, no weight changes.
...
class CLKVKey(Key):
    """Cross-Layer KV Sharing — halve KV cache by sharing across layer pairs.
    ...
    def forward(self, data: dict[str, torch.Tensor]) -> KeyResult:
        """CLKV is a runtime key — state dict is unchanged."""
    ...
    def reverse(self, weights: dict[str, torch.Tensor]) -> KeyResult:
        """No-op — CLKV doesn't modify weights."""
        ...
        def clkv_forward(
            ...
            for i, block in enumerate(model.blocks):
                ...
                if group_pos == 0:
                    ...
                    # so the block doesn't append new tokens to KV — it just uses
