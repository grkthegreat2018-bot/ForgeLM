"""Anchor Vocab Pruning (AnchorVocab) — cluster-based top-K logit projection.

Novel insight: For large-vocabulary models (vocab=151936), the head projection
(hidden @ head.weight.T) dominates single-token decode FLOPs: 1536 × 151936 =
234M FLOPs per token. But we only need the top-K logits for sampling — the
...
Anchor Vocab Pruning:
  ...
  2. ONLINE (per token):
     a. Coarse pass: compute hidden @ centroids.T → (K,) scores
     b. Select top-C clusters by score
     c. Fine pass: compute hidden @ cluster_tokens.T → exact logits for
        only the ~C×(V/K) tokens in the selected clusters
     d. Return sparse logits (top candidates only) for sampling

For ForgeLM V2 (vocab=151936, d_model=1536):
  - K=512 clusters (~297 tokens each), top-8 clusters → ~2376 tokens
  - Coarse: 1536 × 512 = 786K FLOPs
  - Fine:   1536 × 2376 = 3.6M FLOPs
...
Key class: TRIVIAL — runtime optimization, training-free, no weight changes.
...
class AnchorVocabHead(nn.Module):
    ...
    def __init__(self, head_weight: torch.Tensor, centroids: torch.Tensor,
        ...
        """
        Args:
            head_weight: (vocab, d_model) — original head weight (tied with embed)
            centroids: (K, d_model) — cluster centroids
            token_to_cluster: (vocab,) — cluster index for each token
    ...
    def forward(self, hidden: torch.Tensor) -> torch.Tensor:
        ...
        Returns:
            logits: (B, T, vocab) — sparse, with -inf for pruned tokens
        """
        if hidden.dim() == 2:
        ...
        # Step 1: Coarse pass — score against centroids
        # (N, D) @ (D, K) → (N, K)
        ...
        # Step 4: Fine pass — compute exact logits for selected tokens only
        ...
        # safe_members: (N, C*max_cs) — token indices
        # fine_logits: (N, C*max_cs) — logit values
        # valid_mask: (N, C*max_cs) — True for valid entries
...
class AnchorVocabKey(Key):
    """Anchor Vocab Pruning — cluster-based top-K logit projection.
    ...
    Key class: TRIVIAL — runtime optimization, training-free, reversible.
    """
    ...
    def forward(self, data: dict[str, torch.Tensor]) -> KeyResult:
        """AnchorVocab is a runtime key — state dict is unchanged."""
    ...
    def reverse(self, weights: dict[str, torch.Tensor]) -> KeyResult:
        """No-op — AnchorVocab doesn't modify weights."""
    ...
    def _kmeans(self, embeddings: torch.Tensor, k: int, max_iter: int = 20) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        Args:
            embeddings: (V, D) — vocabulary embeddings
            k: number of clusters
            max_iter: iterations

        Returns:
            centroids: (k, D)
            assignments: (V,) — cluster index per token
        """
    ...
    def apply(self, model: nn.Module) -> int:
        ...
        print(f"  [AnchorVocab] Clustering done: avg={avg_size:.0f}, "
              ...
              f"top-{self.top_clusters} → ~{self.top_clusters * avg_size:.0f} candidates "
