...
def build_qwen2_keystack() -> KeyStack:
    ...
    """
    from research.keys.attention.causal_mask_key import CausalMaskKey
    from research.keys.misc.embedding_key import EmbeddingKey
    from research.keys.attention.gqa_key import GQAKey
    from research.keys.misc.lm_head_tied_key import LMHeadTiedKey
    from research.keys.normalization.rmsnorm_key import RMSNormKey
    from research.keys.position.rope_key import RoPEKey
    from research.keys.activation.swiglu_key import SwiGLUKey
...
def build_xp_keystack() -> KeyStack:
    ...
    Classification:
      ...
      - PARTIAL (9): GQA→MQA, Wanda, DSpark, MoE Router, SSA, GateSkip, Liquid Conv,
                      SparDA, PartialRoPE
    """
    from research.keys.activation.activation_transmute_key import ActivationTransmuteKey
    from research.keys.moe.airllm_key import AirLLMKey
    from research.keys.moe.airmoe_key import AirMoEKey

    # --- New keys (Bonsai-27B inspired + self-play pipeline) ---
    from research.keys.quantization.binary_g128_key import BinaryG128Key  # LOSSY: binary {±1} g128, 14.2x compression
    from research.keys.attention.causal_mask_key import CausalMaskKey
    from research.keys.knowledge.context_patch_key import ContextPatchKey
    from research.keys.knowledge.cot_knowledge_pack_key import CoTKnowledgePackKey  # LOSSLESS: CoT KV cache injection
    from research.keys.compression.denseformer_key import DenseFormerKey
    from research.keys.speculative.dspark_key import DSparkKey
    from research.keys.misc.embedding_key import EmbeddingKey
    from research.keys.moe.expert_consolidation_key import ExpertConsolidationKey
    from research.keys.knowledge.fact_injection_key import FactInjectionKey
    from research.keys.misc.gateskip_key import GateSkipKey
    from research.keys.attention.gqa_to_mqa_key import GQAToMQAKey
    from research.keys.knowledge.grail_key import GRAILKey
    from research.keys.attention.hybrid_linear_key import HybridLinearKey  # LOSSY: 75% linear / 25% full attention
    from research.keys.knowledge.knowledge_pack_key import KnowledgePackKey
    from research.keys.quantization.kv4bit_key import KV4BitKey  # LOSSY: 4-bit KV cache with scale absorption
    from research.keys.misc.liquid_conv_key import LiquidConvKey
    from research.keys.misc.lm_head_tied_key import LMHeadTiedKey
    from research.keys.training.logit_cap_key import LogitCapKey
    from research.keys.quantization.lossless_quant_key import LosslessQuantKey
    from research.keys.moe.moe_router_key import MoERouterKey
    from research.keys.misc.mrl_key import MRLKey
    from research.keys.speculative.mtp_key import MTPKey
    from research.keys.normalization.norm_folding_key import NormFoldingKey
    from research.keys.position.partial_rope_key import PartialRoPEKey
    from research.keys.attention.qk_norm_mla_key import QKNormMLAKey
    from research.keys.quantization.quarot_key import QuaRotR2Key
    from research.keys.normalization.rmsnorm_key import RMSNormKey
    from research.keys.position.rope_key import RoPEKey
    from research.keys.quantization.rotorquant_key import RotorQuantKey
    from research.keys.normalization.sandwich_norm_key import SandwichNormKey
    from research.keys.knowledge.self_play_key import SelfPlayKey
    from research.keys.knowledge.selfplay_context_patch_key import SelfPlayContextPatchKey  # LOSSLESS: self-play rank-1 patches
    from research.keys.quantization.slicegpt_key import SliceGPTKey
    from research.keys.quantization.sparda_key import SparDAKey
    from research.keys.quantization.spinquant_key import SpinQuantHadamardKey
    from research.keys.attention.ssa_key import SSAKey
    from research.keys.activation.swiglu_clamp_key import SwiGLUClampKey
    from research.keys.knowledge.test_gated_injection_key import TestGatedFactInjectionKey  # LOSSLESS: test-verified fact injection
    from research.keys.misc.value_residual_key import ValueResidualKey
    from research.keys.compression.wanda_key import WandaKey
    from research.keys.compression.wq_elim_key import WQElimKey
    ...
    from research.keys.attention.attn_scale_fold_key import AttnScaleFoldKey
    from research.keys.compression.dead_weight_key import DeadWeightKey
    from research.keys.position.rope_share_key import RoPEShareKey
    from research.keys.compression.tensor_dedup_key import TensorDedupKey
